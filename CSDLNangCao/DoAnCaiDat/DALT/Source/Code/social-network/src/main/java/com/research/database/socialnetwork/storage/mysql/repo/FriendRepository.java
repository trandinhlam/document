...
public interface FriendRepository extends CrudRepository<Friend, Integer> {
    ...
    List<Friend> findByUid(@Param("userId") Integer userId, @Param("trangthai") int trangthai);

    @Query("select f from Friend f where (f.receiver=:receiver and f.trangthai=:trangthai)")
    List<Friend> findWaitings(@Param("receiver") Integer receiver, @Param("trangthai") int trangthai);

    @Query("select f from Friend f where (f.inviter=:inviter and f.trangthai=:trangthai)")
    List<Friend> findRespondings(@Param("inviter") Integer inviter, @Param("trangthai") int trangthai);

    @Query("select f from Friend f where (f.inviter=:user1 and f.receiver=:user2) or (f.inviter=:user2 and f.receiver=:user1)")
    Collection<Friend> findByKey(@Param("user1") int user1, @Param("user2") int user2);
